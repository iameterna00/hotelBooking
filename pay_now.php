...
if (isset($_POST['pay_now'])) {
    ...
    $query1 = "INSERT INTO `booking_order` (`user_id`, `room_id`, `check_in`, `check_out`, `booking_status`, `order_id`, `trans_amt`) 
               VALUES (?, ?, ?, ?, 'booked', ?, ?)";
    ...
    if ($result1) {
        ...
        if ($result2) {
            ...
            log_error("Error inserting booking details: " . mysqli_error($con));
            echo "Error inserting booking details.";
        }
    } else {
        log_error("Error inserting booking order: " . mysqli_error($con));
        echo "Error inserting booking order.";
    }
}

function log_error($message) {
    $file = 'error_log.txt'; // Make sure this file is writable
